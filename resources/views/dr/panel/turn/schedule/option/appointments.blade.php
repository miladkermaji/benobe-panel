...
function getPaymentStatus(status) {
    switch (status) {
        case 'pending': return 'درحال پرداخت';
...
const appointmentsContainer = $('#appointment-lists-container');
    ...
    $('.appointments-filter-drop-toggle li').on('click', function () {
        ...
        $.ajax({
            ...
            type: "GET",
            data: {
                type: filterTypeText === 'کل نوبت ها' ? '' : (filterTypeText === 'نوبت های مطب' ? 'in_person' : 'online'),
                date: selectedDate,
                selectedClinicId: selectedClinicId
            },
            success: function (response) {
...
$(document).ready(function () {
    ...
    searchInput.on('input', function () {
        ...
        if (!selectedDate) {
            ...
            appointmentsContainer.html('<div class="text-center w-100 text-danger">لطفاً ابتدا یک تاریخ انتخاب کنید.</div>');
            return;
        }
    ...
    function searchAppointments(query, selectedClinicId, filterText, date) {
        ...
        const filterType = filterText === 'کل نوبت ها' ? '' : (filterText === 'نوبت های مطب' ? 'in_person' : 'online');
