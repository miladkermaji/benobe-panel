                ...
                <div class="secretary-toggle border-bottom">
                    <div class="d-flex justify-content-between align-items-center cursor-pointer toggle-header"
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <span class="fw-bold" style="color: var(--text-primary);">{{ $secretary->first_name }} {{ $secretary->last_name }}</span>
                            ...
                            @foreach ($permissions as $permissionKey => $permissionData)
                                <div class="permission-item rounded">
                                    <div class="form-check">
                                        <input class="form-check-input parent-permission update-permissions"
                                               type="checkbox" id="perm-{{ $secretary->id }}-{{ $permissionKey }}"
                                               data-secretary-id="{{ $secretary->id }}"
                                               value="{{ $permissionKey }}"
                                               {{ in_array($permissionKey, $savedPermissions) ? 'checked' : '' }}>
                                        <label class="form-check-label fw-bold"
                                               for="perm-{{ $secretary->id }}-{{ $permissionKey }}"
                                    ...
                                    @if (!empty($permissionData['routes']))
                                        <div class="permission-sub-items">
                                            @foreach ($permissionData['routes'] as $routeKey => $routeTitle)
                                                <div class="form-check">
                                                    <input class="form-check-input child-permission update-permissions"
                                                           type="checkbox"
                                                    ...
                                                    <label class="form-check-label"
                                                           for="perm-{{ $secretary->id }}-{{ $routeKey }}"
