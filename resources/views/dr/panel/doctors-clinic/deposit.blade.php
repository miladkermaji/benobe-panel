...
<style>
    ...
    .table {
        ...
        direction: rtl; /* جهت جدول از راست به چپ */
    }
    ...
    .table tr {
        display: flex;
        flex-direction: row-reverse; /* معکوس کردن جهت از چپ به راست */
    }

    .table th,
    .table td {
        flex: 1; /* تقسیم فضای برابر بین ستون‌ها */
    }

    /* تنظیم عرض ستون عملیات */
    .table th:first-child,
    .table td:first-child {
        flex: 0 0 auto; /* عرض ثابت برای ستون عملیات */
        width: 120px; /* عرض مشخص برای دکمه‌ها */
    }
...
<div class="container">
    <div class="header">
        ...
        <button class="btn btn-primary" id="addDepositBtn">+ افزودن بیعانه</button>
    </div>
    <div class="card">
        <table class="table">
            <thead>
    ...
    <div class="modal-content">
        ...
        <form id="depositForm">
            ...
            <div class="form-group" id="customPriceContainer" style="display: none;">
                ...
                <input type="number" name="custom_price" id="customPrice" class="form-control" placeholder="مبلغ را وارد کنید" min="0">
            </div>
            ...
            <button type="submit" class="btn btn-primary" style="width: 100%; margin-top: 16px;">ذخیره</button>
        </form>
    </div>
</div>
@endsection
...
$(document).ready(function() {
    ...
    noDepositCheckbox.on('change', function() {
        ...
        customPriceContainer.css('display', this.checked ? 'none' : (depositSelect.val() === 'custom' ? 'block' : 'none'));
    });
    ...
    form.on('submit', async function(e) {
        ...
        const url = id ? `{{ route('doctors.clinic.deposit.update', '') }}/${id}` : '{{ route('doctors.clinic.deposit.store') }}';
    ...
    $(document).on('click', '.edit-btn', function() {
        ...
        const amount = row.find('td:eq(2)').text().trim() === 'بدون بیعانه' ? '' : row.find('td:eq(2)').text().replace(/,/g, ''); // تغییر به td:eq(2) چون مبلغ حالا ستون سومه
        modal.css('display', 'flex');
