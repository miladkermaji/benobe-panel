<div class="segmented-toggle {{ $attributes->get('class') }}" dir="ltr">
  <div class="toggle-wrapper">
    ...
    <div class="toggle-segments">
      ...
      <label class="toggle-segment toggle-segment-on {{ $isChecked ? 'active' : '' }}" for="{{ $id }}" data-value="true">
        آنلاین + دستی
      </label>
    </div>
  </div>
...
<style>
 ...
 .segmented-toggle {
  --bs-toggle-width: 240px; /* عرض بزرگ‌تر برای حس پریمیوم */
  --bs-toggle-height: 48px; /* ارتفاع مدرن */
  --bs-toggle-font-size: 0.95rem; /* فونت خوانا و شیک */
  --bs-toggle-border-radius: 14px; /* گوشه‌های گرد نرم */
  --bs-toggle-bg: rgba(255, 255, 255, 0.85); /* پس‌زمینه شیشه‌ای */
  --bs-toggle-active-bg: var(--gradient-primary); /* گرادیان بنفش-صورتی نئونی */
  --bs-toggle-color: #475569; /* رنگ متن غیرفعال */
  --bs-toggle-active-color: #ffffff; /* رنگ متن فعال */
  --bs-toggle-shadow: 0 4px 15px rgba(0, 0, 0, 0.1), 0 2px 8px rgba(0, 0, 0, 0.05); /* سایه‌های چندلایه */
  --bs-toggle-backdrop-blur: blur(12px); /* افکت بلور شیشه‌ای */
  --bs-toggle-transition: all 0.4s cubic-bezier(0.68, -0.55, 0.265, 1.55); /* انیمیشن فنری و پویا */
}
...
.segmented-toggle .toggle-wrapper {
  ...
  padding: 4px;
  background: rgba(255, 255, 255, 0.1); /* لایه پایه شیشه‌ای */
  border-radius: var(--bs-toggle-border-radius);
  backdrop-filter: var(--bs-toggle-backdrop-blur); /* افکت بلور */
...
.segmented-toggle .toggle-segments {
  ...
  border-radius: var(--bs-toggle-border-radius);
  overflow: hidden;
  border: 1px solid rgba(255, 255, 255, 0.2); /* حاشیه ظریف */
...
.segmented-toggle .toggle-segment {
  ...
  font-weight: 600; /* فونت کمی ضخیم برای حس مدرن */
...
.segmented-toggle .toggle-segment.active {
  ...
  box-shadow: 0 3px 10px rgba(168, 85, 247, 0.4); /* سایه بنفش ملایم */
  transform: translateY(-2px) scale(1.03); /* حس بالا آمدن و بزرگ‌نمایی */
}

.segmented-toggle .toggle-segment:hover {
  background: rgba(241, 245, 249, 0.8); /* هاور شیشه‌ای ملایم */
  color: #1e293b;
  transform: scale(1.01); /* میکرواینتراکشن هاور */
}

.segmented-toggle .toggle-segment.active:hover {
  background: var(--bs-toggle-active-bg);
  filter: brightness(1.15); /* روشن‌تر شدن در هاور */
}

.segmented-toggle .toggle-input:focus + .toggle-segments .toggle-segment.active {
  outline: 2px solid #a855f7; /* فکوس بنفش برای دسترسی‌پذیری */
  outline-offset: 2px;
  border-radius: var(--bs-toggle-border-radius);
}
...
.segmented-toggle .toggle-segment-on::before {
  ...
  bottom: 8px;
  width: 1px;
  background: rgba(71, 85, 105, 0.25); /* جداکننده ظریف */
  transition: var(--bs-toggle-transition);
}
...
.segmented-toggle .toggle-segment.active::after {
  ...
  inset: 0;
  border-radius: var(--bs-toggle-border-radius);
  box-shadow: 0 0 12px rgba(168, 85, 247, 0.5); /* درخشش نئونی */
  z-index: -1;
  opacity: 0.6;
  transition: var(--bs-toggle-transition);
}
