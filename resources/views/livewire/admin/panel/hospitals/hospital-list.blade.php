            ...
            <table class="table table-bordered table-hover w-100 m-0">
              <thead class="glass-header text-white">
                <tr>
                  ...
                  <th class="text-center align-middle" style="width: 100px;">گالری</th>
              ...
              <tbody>
                ...
                @else
                  <tr>
                    <td colspan="12" class="text-center py-5">در حال بارگذاری بیمارستان‌ها...</td>
                  </tr>
                @endif
              </tbody>
            </table>
                ...
                <div class="card-body p-3">
                  <div class="d-flex justify-content-between align-items-center mb-2">
                    <div class="d-flex align-items-center gap-2">
                      <input type="checkbox" wire:model.live="selectedHospitals" value="{{ $item->id }}"
                        class="form-check-input m-0">
                      <span class="badge bg-label-primary">#{{ $hospitals->firstItem() + $index }}</span>
                    </div>
                    <button wire:click="toggleStatus({{ $item->id }})"
                      class="badge {{ $item->is_active ? 'bg-label-success' : 'bg-label-danger' }} border-0 cursor-pointer">
                      {{ $item->is_active ? 'فعال' : 'غیرفعال' }}
                  ...
                  <div class="mb-2">
                    <small class="text-muted d-block mb-1">نام:</small>
                  ...
                  <div class="mb-2">
                    <small class="text-muted d-block mb-1">استان:</small>
                    <span class="fw-medium">{{ $item->province?->name ?? '-' }}</span>
                  </div>
                  <div class="mb-3">
                    <small class="text-muted d-block mb-1">شهر:</small>
            ...
            @empty
              <div class="text-center py-5">
                <div class="d-flex flex-column align-items-center justify-content-center">
                  ...
                  <p class="text-muted fw-medium m-0">هیچ بیمارستانی یافت نشد.</p>
                </div>
              </div>
            @endforelse
          @else
            <div class="text-center py-5">در حال بارگذاری بیمارستان‌ها...</div>
          @endif
        </div>
  ...
  <script>
    document.addEventListener('livewire:init', function() {
      ...
      Livewire.on('confirm-delete', (event) => {
        Swal.fire({
          title: 'حذف بیمارستان',
          text: 'آیا مطمئن هستید که می‌خواهید این بیمارستان را حذف کنید؟',
          icon: 'warning',
