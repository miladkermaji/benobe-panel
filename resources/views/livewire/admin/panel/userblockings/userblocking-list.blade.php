<div class="container-fluid py-2" dir="rtl" wire:init="loadUserBlockings">
    <div class="glass-header text-white p-3 rounded-3 mb-5 shadow-lg d-flex justify-content-between align-items-center flex-wrap gap-3">
        <h1 class="m-0 h3 font-thin flex-grow-1" style="min-width: 200px;">مدیریت کاربران مسدود</h1>
        <div class="input-group flex-grow-1 position-relative" style="max-width: 400px;">
            <input type="text" class="form-control border-0 shadow-none bg-white text-dark ps-5 rounded-3"
                wire:model.live="search" placeholder="جستجو در کاربران مسدود..." style="padding-right: 23px">
            ...
            <div class="card-body p-0">
                <div class="table-responsive text-nowrap">
                    <table class="table table-bordered table-hover w-100 m-0">
                        ...
                        <tbody>
                                ...
                                @forelse ($userBlockings as $index => $item)
                                    <tr>
                                        <td class="text-center align-middle">
                                            <input type="checkbox" wire:model.live="selectedUserBlockings"
                                                value="{{ $item->id }}" class="form-check-input m-0">
                                        </td>
                                        ...
                                        <td class="text-center align-middle">
                                            <div class="d-flex justify-content-center gap-2">
                                                <a href="{{ route('admin.panel.userblockings.edit', $item->id) }}"
                            ...
                            @else
                                <tr>
                                    <td colspan="9" class="text-center py-5">در حال بارگذاری کاربران مسدود...</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
                ...
                <div class="d-flex justify-content-between align-items-center mt-4 px-4 flex-wrap gap-3">
                    <div class="text-muted">نمایش {{ $userBlockings ? $userBlockings->firstItem() : 0 }} تا
                        {{ $userBlockings ? $userBlockings->lastItem() : 0 }} از {{ $userBlockings ? $userBlockings->total() : 0 }} ردیف
                    </div>
                    @if ($userBlockings && $userBlockings->hasPages())
                        {{ $userBlockings->links('livewire::bootstrap') }}
                    @endif
                </div>
            </div>
        </div>
        ...
        document.addEventListener('livewire:init', function () {
            ...
            Livewire.on('confirm-delete', (event) => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        Livewire.dispatch('deleteUserBlockingConfirmed', { id: event.id });
                    }
                });
            });
            ...
            Livewire.on('confirm-toggle-status', (event) => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        Livewire.dispatch('toggleStatusConfirmed', { id: event.id });
                    }
                });
            });
        });
