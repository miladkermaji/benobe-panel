    ...
    <div class="card shadow-lg border-0 rounded-3 overflow-hidden">
        <div class="card-header bg-gradient-primary text-white p-4 d-flex align-items-center justify-content-between flex-wrap gap-3">
            <div class="d-flex align-items-center gap-3">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="custom-animate-bounce">
                    <path d="M5 12h14M12 5l7 7-7 7" />
                </svg>
                <h5 class="mb-0 fw-bold text-shadow">ویرایش کلینیک: {{ $name }}</h5>
            </div>
            ...
            <div class="row justify-content-center">
                <div class="col-12 col-md-10 col-lg-8">
                    <div class="row g-4">
                        ...
                        <div class="col-6 col-md-6 position-relative mt-5">
                            <input type="text" wire:model="secretary_phone" class="form-control" id="secretary_phone" placeholder=" ">
                        ...
                        <div class="col-6 col-md-6 position-relative mt-5">
                            <input type="number" wire:model="latitude" class="form-control" id="latitude" placeholder=" " step="0.0000001">
                            <label for="latitude" class="form-label">عرض جغرافیایی</label>
                        </div>
                        <div class="col-6 col-md-6 position-relative mt-5">
                            <input type="number" wire:model="longitude" class="form-control" id="longitude" placeholder=" " step="0.0000001">
                        ...
                        <div class="col-6 col-md-6 position-relative mt-5 d-flex align-items-center">
                            <div class="form-check form-switch w-100 d-flex align-items-center">
                                <input class="form-check-input" type="checkbox" id="location_confirmed" wire:model="location_confirmed">
                        ...
                        <div class="col-12 position-relative mt-5">
                            ...
                            <div class="row g-3">
                                @foreach (['monday' => 'دوشنبه', 'tuesday' => 'سه‌شنبه', 'wednesday' => 'چهارشنبه', 'thursday' => 'پنج‌شنبه', 'friday' => 'جمعه', 'saturday' => 'شنبه', 'sunday' => 'یک‌شنبه'] as $day => $label)
                                    <div class="col-6 col-md-3">
                                        <div class="form-check p-3 bg-light rounded shadow-sm hover:bg-gray-100 transition-all">
                                            <input class="form-check-input" type="checkbox" wire:model="working_days.{{ $day }}" id="working_days_{{ $day }}" value="{{ $day }}">
                                            <label class="form-check-label fw-medium text-dark" for="working_days_{{ $day }}">{{ $label }}</label>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="col-12 position-relative mt-5">
                            ...
                            @if ($clinic->avatar)
                                <img src="{{ Storage::url($clinic->avatar) }}" alt="تصویر اصلی" class="mt-2" style="max-width: 100px;">
                            @endif
                        </div>
                        <div class="col-12 position-relative mt-5">
                            ...
                            @if ($clinic->documents)
                                <div class="mt-2">
                                    @foreach ($clinic->documents as $document)
                                        <a href="{{ Storage::url($document) }}" target="_blank" class="d-block">{{ basename($document) }}</a>
                                    @endforeach
                                </div>
                            @endif
                        </div>
                            ...
                            <div class="phone-numbers">
                                @foreach ($phone_numbers as $index => $phone)
                                    <div class="input-group mb-2" wire:ignore.self>
                                        <input type="text" wire:model="phone_numbers.{{ $index }}" class="form-control" placeholder="شماره تماس {{ $index + 1 }}">
                                        <button class="btn btn-outline-danger" type="button" wire:click="removePhoneNumber({{ $index }})">حذف</button>
                                    </div>
                                @endforeach
                                <button type="button" wire:click="addPhoneNumber" class="btn btn-outline-primary mt-2">افزودن شماره تماس</button>
                            </div>
                        </div>
                        <div class="col-12 position-relative mt-5">
                        ...
                        <div class="text-end mt-4 w-100 d-flex justify-content-end">
                            <button wire:click="update" class="btn my-btn-primary px-5 py-2 d-flex align-items-center gap-2 shadow-lg hover:shadow-xl transition-all">
