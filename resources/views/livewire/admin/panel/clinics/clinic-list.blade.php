  ...
  <div
    ...
    <div class="input-group flex-grow-1 position-relative" style="max-width: 400px;">
      <input type="text" class="form-control border-0 shadow-none bg-white text-dark ps-5 rounded-3"
        wire:model.live="search" placeholder="جستجو در کلینیک ها..." style="padding-right: 23px">
          ...
          <div class="table-responsive text-nowrap">
            <table class="table table-bordered table-hover w-100 m-0">
              ...
              <tbody>
                @if ($readyToLoad)
                  @forelse ($clinics as $index => $item)
                    <tr>
                      ...
                      <td class="text-center align-middle">{{ $clinics->firstItem() + $index }}</td>
                ...
                @else
                  <tr>
                    <td colspan="11" class="text-center py-5">در حال بارگذاری کلینیک ها...</td>
                  </tr>
                @endif
              </tbody>
            </table>
                ...
                <div class="card-body p-3">
                  <div class="d-flex justify-content-between align-items-center mb-2">
                    <div class="d-flex align-items-center gap-2">
                      <input type="checkbox" wire:model.live="selectedClinics" value="{{ $item->id }}"
                        class="form-check-input m-0">
                      <span class="badge bg-label-primary">#{{ $clinics->firstItem() + $index }}</span>
                    </div>
                    <button wire:click="toggleStatus({{ $item->id }})"
                      class="badge {{ $item->is_active ? 'bg-label-success' : 'bg-label-danger' }} border-0 cursor-pointer">
                      {{ $item->is_active ? 'فعال' : 'غیرفعال' }}
            ...
            @empty
              <div class="text-center py-5">
                <div class="d-flex flex-column align-items-center justify-content-center">
                  ...
                  <p class="text-muted fw-medium m-0">هیچ کلینیکی یافت نشد.</p>
                </div>
              </div>
            @endforelse
          @else
            <div class="text-center py-5">در حال بارگذاری کلینیک ها...</div>
          @endif
        </div>
        <div class="d-flex justify-content-between align-items-center mt-4 px-4 flex-wrap gap-3">
          <div class="text-muted">نمایش {{ $clinics ? $clinics->firstItem() : 0 }} تا
            {{ $clinics ? $clinics->lastItem() : 0 }} از {{ $clinics ? $clinics->total() : 0 }} ردیف
          </div>
          @if ($clinics && $clinics->hasPages())
