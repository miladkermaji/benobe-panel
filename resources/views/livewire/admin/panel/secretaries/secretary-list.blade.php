<div class="container-fluid py-2" dir="rtl" wire:init="loadSecretaries">
    <div class="glass-header text-white p-3 rounded-3 mb-5 shadow-lg d-flex justify-content-between align-items-center flex-wrap gap-3">
        <h1 class="m-0 h3 font-thin flex-grow-1" style="min-width: 200px;">مدیریت منشی‌ها</h1>
        <div class="input-group flex-grow-1 position-relative" style="max-width: 400px;">
            <input type="text" class="form-control border-0 shadow-none bg-white text-dark ps-5 rounded-3 px-4" wire:model.live="search" placeholder="جستجو در منشی‌ها یا پزشکان...">
        ...
        <div class="d-flex gap-2 flex-shrink-0 flex-wrap justify-content-center mt-md-2 buttons-container">
            <a href="{{ route('admin.panel.secretaries.create') }}" class="btn btn-gradient-success rounded-pill px-4 d-flex align-items-center gap-2">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M12 5v14M5 12h14" />
                </svg>
                <span>افزودن منشی</span>
            </a>
        </div>
    </div>
        ...
        <div class="card shadow-sm">
            <div class="card-body p-0">
                @if ($readyToLoad)
                        ...
                        <div class="doctor-toggle border-bottom">
                            <div class="d-flex justify-content-between align-items-center p-3 cursor-pointer" wire:click="toggleDoctor({{ $doctor->id }})">
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <span class="badge bg-label-primary">{{ $doctor->secretaries->count() }} منشی</span>
                                </div>
                                        ...
                                        <tbody>
                                            @forelse ($doctor->secretaries as $index => $secretary)
                                                <tr>
                                                    ...
                                                    <td class="align-middle">{{ $secretary->first_name . ' ' . $secretary->last_name }}</td>
                                                    <td class="align-middle">{{ $secretary->mobile }}</td>
                                                    <td class="align-middle">{{ $secretary->clinic ? $secretary->clinic->name : '-' }}</td>
                                                    <td class="text-center align-middle">
                                                        <button wire:click="toggleStatus({{ $secretary->id }})"
                                                            class="badge {{ $secretary->is_active ? 'bg-label-success' : 'bg-label-danger' }} border-0 cursor-pointer">
                                                ...
                                                <tr>
                                                    <td colspan="6" class="text-center py-5">
                                                        <div class="d-flex flex-column align-items-center justify-content-center">
                                                            ...
                                                            <p class="text-muted fw-medium m-0">هیچ منشی‌ای یافت نشد.</p>
                                                        </div>
                                                    </td>
                                                </tr>
                                            @endforelse
                    ...
                    @endforelse
                @else
                    <div class="text-center py-5">در حال بارگذاری منشی‌ها...</div>
                @endif
            </div>
        </div>
    </div>
            ...
            Livewire.on('confirm-delete', (event) => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        Livewire.dispatch('deleteSecretary', { id: event.id });
                    }
                });
            });
        });
