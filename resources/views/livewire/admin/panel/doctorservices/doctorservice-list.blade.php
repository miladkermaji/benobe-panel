<div class="container-fluid py-2" dir="rtl" wire:init="loadDoctorServices">
 <div
  ...
  <div class="d-flex gap-2 flex-shrink-0 flex-wrap justify-content-center mt-md-2 buttons-container">
   <a href="{{ route('admin.panel.doctorservices.create') }}"
  ...
  <div class="card shadow-sm">
   <div class="card-body p-0">
    <div class="table-responsive text-nowrap">
     <table class="table table-bordered table-hover w-100 m-0">
      ...
      <tbody>
        ...
        @forelse ($doctorservices as $index => $item)
         <tr>
          <td class="text-center align-middle">
           <input type="checkbox" wire:model.live="selectedDoctorServices" value="{{ $item->id }}"
            class="form-check-input m-0">
          </td>
          <td class="text-center align-middle">{{ $doctorservices->firstItem() + $index }}</td>
          <td class="align-middle">{{ $item->doctor->first_name . ' ' . $item->doctor->last_name ?? 'نامشخص' }}</td>
          ...
          <td class="text-center align-middle">
           <div class="d-flex justify-content-center gap-2">
            <a href="{{ route('admin.panel.doctorservices.edit', $item->id) }}"
         ...
         <tr>
          <td colspan="11" class="text-center py-5">
           <div class="d-flex justify-content-center align-items-center flex-column">
            ...
            <p class="text-muted fw-medium">هیچ خدمت پزشکی یافت نشد.</p>
           </div>
          </td>
         </tr>
        @endforelse
       @else
        <tr>
         <td colspan="11" class="text-center py-5">در حال بارگذاری خدمات پزشکان...</td>
        </tr>
       @endif
      </tbody>
     </table>
    ...
    <div class="d-flex justify-content-between align-items-center mt-4 px-4 flex-wrap gap-3">
     <div class="text-muted">نمایش {{ $doctorservices ? $doctorservices->firstItem() : 0 }} تا
      {{ $doctorservices ? $doctorservices->lastItem() : 0 }} از {{ $doctorservices ? $doctorservices->total() : 0 }}
      ردیف
     </div>
