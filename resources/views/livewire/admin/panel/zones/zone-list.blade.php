<div class="container-fluid py-2" dir="rtl" wire:init="loadZones">
    <div class="glass-header text-white p-3 rounded-3 mb-5 shadow-lg d-flex justify-content-between align-items-center flex-wrap gap-3">
        ...
        <div class="input-group flex-grow-1 position-relative" style="max-width: 400px;">
            <input type="text" class="form-control border-0 shadow-none bg-white text-dark ps-5 rounded-3" wire:model.live="search" placeholder="جستجو در استان‌ها..." style="padding-right: 23px">
        ...
        <div class="d-flex gap-2 flex-shrink-0 flex-wrap justify-content-center mt-md-2 buttons-container">
            <a href="{{ route('admin.panel.zones.create') }}" class="btn btn-gradient-success rounded-pill px-4 d-flex align-items-center gap-2">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M12 5v14M5 12h14" />
                </svg>
                <span>افزودن استان</span>
            </a>
            <button wire:click="deleteSelected" class="btn btn-gradient-danger rounded-pill px-4 d-flex align-items-center gap-2" @if (empty($selectedZones)) disabled @endif>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="card shadow-sm">
            <div class="card-body p-0">
                                ...
                                @forelse ($zones as $index => $item)
                                    <tr>
                                        <td class="text-center align-middle">
                                            <input type="checkbox" wire:model.live="selectedZones" value="{{ $item->id }}" class="form-check-input m-0">
                                        </td>
                                        ...
                                        <td class="align-middle">{{ $item->price_shipping ? number_format($item->price_shipping) . ' تومان' : '-' }}</td>
                                        <td class="text-center align-middle">
                                            <button wire:click="toggleStatus({{ $item->id }})" class="badge {{ $item->status ? 'bg-label-success' : 'bg-label-danger' }} border-0 cursor-pointer">
                                                {{ $item->status ? 'فعال' : 'غیرفعال' }}
                                            </button>
                                        </td>
                                        <td class="text-center align-middle">
                                            <div class="d-flex justify-content-center gap-2">
                                                <a href="{{ route('admin.panel.zones.edit', $item->id) }}" class="btn btn-gradient-success rounded-pill px-3">
                                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                ...
                                                <a href="{{ route('admin.panel.cities.index', ['province_id' => $item->id]) }}" class="btn btn-light rounded-pill px-3" title="مشاهده شهرها">
                                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <div class="d-flex justify-content-between align-items-center mt-4 px-4 flex-wrap gap-3">
                    <div class="text-muted">نمایش {{ $zones ? $zones->firstItem() : 0 }} تا {{ $zones ? $zones->lastItem() : 0 }} از {{ $zones ? $zones->total() : 0 }} ردیف</div>
            ...
            Livewire.on('confirm-delete', (event) => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        Livewire.dispatch('deleteZoneConfirmed', { id: event.id });
                    }
                });
            });
        });
