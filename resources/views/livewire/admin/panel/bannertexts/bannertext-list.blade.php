    ...
    <div class="glass-header text-white p-3 rounded-3 mb-5 shadow-lg d-flex justify-content-between align-items-center flex-wrap gap-3">
        ...
        <div class="input-group flex-grow-1 position-relative" style="max-width: 400px;">
            <input type="text" class="form-control border-0 shadow-none bg-white text-dark ps-5 rounded-3"
                wire:model.live="search" placeholder="جستجو در بنرها..." style="padding-right: 23px">
        ...
        <div class="d-flex gap-2 flex-shrink-0 flex-wrap justify-content-center mt-md-2 buttons-container">
            <a href="{{ route('admin.panel.bannertexts.create') }}"
            ...
            <button wire:click="deleteSelected"
                ...
                @if (empty($selectedbannertexts)) disabled @endif>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <div class="table-responsive text-nowrap">
                    <table class="table table-bordered table-hover w-100 m-0">
                        ...
                        <tbody>
                                ...
                                @forelse ( $bannertexts as $index => $item )
                                    <tr>
                                        <td class="text-center align-middle">
                                            <input type="checkbox" wire:model.live="selectedbannertexts"
                                                value="{{ $item->id }}" class="form-check-input m-0">
                                        </td>
                                        <td class="text-center align-middle">{{ $bannertexts->firstItem() + $index }}</td>
                                        <td class="align-middle">{{ $item->main_text }}</td>
                                        <td class="align-middle">{{ $item->switch_words ? implode(', ', $item->switch_words) : '-' }}</td>
                                        <td class="align-middle">{{ $item->switch_interval ?? '-' }}</td>
                                        <td class="align-middle">
                                            @if ($item->image_path)
                                                <img src="{{ asset('storage/' . $item->image_path) }}" alt="تصویر بنر" style="max-width: 50px; border-radius: 5px;">
                                            @else
                                                -
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-center align-middle">
                                            <div class="d-flex justify-content-center gap-2">
                                                <a href="{{ route('admin.panel.bannertexts.edit', $item->id) }}"
                            ...
                            @else
                                <tr>
                                    <td colspan="8" class="text-center py-5">در حال بارگذاری بنرها...</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
            ...
            Livewire.on('confirm-delete', (event) => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        Livewire.dispatch('deleteBannerTextConfirmed', { id: event.id });
                    }
                });
            });
        });
