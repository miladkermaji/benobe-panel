<div class="container-fluid py-4" dir="rtl">
    <div class="card shadow-lg border-0 rounded-3 overflow-hidden">
        <div class="card-header bg-gradient-primary text-white p-3 d-flex align-items-center justify-content-between flex-wrap gap-3">
            ...
            <a href="{{ route('admin.panel.laboratories.index') }}" class="btn btn-outline-light btn-sm rounded-pill d-flex align-items-center gap-2 hover:shadow-md transition-all">
            ...
            <div class="row g-4">
                <div class="col-12">
                    <div class="bg-light rounded-3 p-3 shadow-sm hover:shadow-md transition-all">
                        ...
                        @endforeach
                        <button wire:click="uploadImages" class="btn btn-primary rounded-pill mt-3 px-4 d-flex align-items-center gap-2">
                        ...
                        @forelse ($galleries as $gallery)
                            <div class="col-md-3 col-sm-6">
                                <div class="card shadow-sm position-relative">
                                    <img src="{{ Storage::url($gallery->image_path) }}" class="card-img-top" alt="{{ $gallery->caption ?? 'تصویر آزمایشگاه' }}" style="height: 150px; object-fit: cover;">
                                    <div class="card-body p-2 text-center">
                                        <p class="text-muted small">{{ $gallery->caption ?? 'بدون توضیح' }}</p>
                                        <div class="d-flex justify-content-center gap-2">
                                            <button wire:click="setPrimary({{ $gallery->id }})" class="btn btn-sm btn-outline-success rounded-pill {{ $gallery->is_primary ? 'active' : '' }}">
                                                {{ $gallery->is_primary ? 'اصلی' : 'تنظیم به‌عنوان اصلی' }}
                                            </button>
                                            <button wire:click="deleteImage({{ $gallery->id }})" class="btn btn-sm btn-outline-danger rounded-pill">
