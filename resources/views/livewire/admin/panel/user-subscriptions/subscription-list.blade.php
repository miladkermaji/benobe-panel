                    ...
                    <div class="d-flex flex-column flex-md-row align-items-stretch align-items-md-center gap-2 w-100">
                        <div class="d-flex gap-2 flex-shrink-0 justify-content-center w-100 flex-column flex-md-row">
                            <div class="search-container position-relative flex-grow-1 mb-2 mb-md-0 w-100">
                                <input type="text" class="form-control search-input border-0 shadow-none bg-white text-dark ps-4 rounded-2 text-start w-100" wire:model.live="search" placeholder="جستجو بر اساس نام، نام خانوادگی یا موبایل..." style="padding-right: 20px; text-align: right; direction: rtl; width: 100%; max-width: 400px; min-width: 200px;">
                                <span class="search-icon position-absolute top-50 start-0 translate-middle-y ms-2" style="z-index: 5; top: 50%; right: 8px;">
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#6b7280" stroke-width="2">
                                        <path d="M11 3a8 8 0 100 16 8 8 0 000-16zm0 14a6 6 0 110-12 6 6 0 010 12zm5-1l5 5" />
                                    </svg>
                                </span>
                            </div>
                            <a href="{{ route('admin.panel.user-subscriptions.create') }}" class="btn btn-gradient-success btn-gradient-success-576 rounded-1 px-3 py-1 d-flex align-items-center gap-1 w-100 w-md-auto justify-content-center justify-content-md-start">
                                <svg style="transform: rotate(180deg)" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="container-fluid px-0">
            <div class="card shadow-sm rounded-2">
                <div class="card-body p-0">
                    <!-- Group Actions -->
                    <div class="group-actions p-2 border-bottom" x-data="{ show: false }" x-show="$wire.selectedSubscriptions.length > 0 || $wire.applyToAllFiltered">
                        ...
                        <table class="table table-hover w-100 m-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center align-middle cursor-pointer" style="width: 60px;" wire:click="sortBy('id')">ردیف
                                        @if ($sortField === 'id')
                                    ...
                                    @forelse ($subscriptions as $index => $subscription)
                                        <tr class="align-middle">
                                            <td class="text-center">
                                                <div class="d-flex justify-content-center align-items-center">
                                                    <input type="checkbox" wire:model.live="selectedSubscriptions" value="{{ $subscription->id }}" class="form-check-input m-0 align-middle">
                                                </div>
                                            </td>
                                            ...
                                            <td class="text-center align-middle">
                                                <button wire:click="toggleStatus({{ $subscription->id }})" class="badge {{ $subscription->status ? 'bg-label-success' : 'bg-label-danger' }} border-0 cursor-pointer">
                                                    {{ $subscription->status ? 'فعال' : 'غیرفعال' }}
                                                </button>
                                            </td>
                                            <td class="text-center align-middle">
                                                <div class="d-flex justify-content-center gap-2">
                                                    <a href="{{ route('admin.panel.user-subscriptions.edit', $subscription->id) }}" class="btn btn-gradient-primary rounded-pill px-3">
                            ...
                            @forelse ($subscriptions as $index => $subscription)
                                <div class="note-card mb-2" x-data="{ open: false }">
                                    <div class="note-card-header d-flex justify-content-between align-items-center px-2 py-2" @click="open = !open" style="cursor:pointer;">
                                        <span class="fw-bold">{{ $subscription->user->full_name ?? 'نامشخص' }} <span class="text-muted">({{ $subscription->plan->name ?? 'نامشخص' }})</span></span>
                                    ...
                                    <div class="note-card-body px-2 py-2" x-show="open" x-transition>
                                        ...
                                        <div class="note-card-item d-flex justify-content-between align-items-center py-1">
                                            ...
                                            <span class="note-card-value">{{ \Morilog\Jalali\Jalalian::fromCarbon(\Carbon\Carbon::parse($subscription->start_date))->format('Y/m/d') }}</span>
                                        </div>
                                        <div class="note-card-item d-flex justify-content-between align-items-center py-1">
                                            <span class="note-card-label">تاریخ پایان:</span>
                                            <span class="note-card-value">{{ \Morilog\Jalali\Jalalian::fromCarbon(\Carbon\Carbon::parse($subscription->end_date))->format('Y/m/d') }}</span>
                                        </div>
                                        <div class="note-card-item d-flex justify-content-between align-items-center py-1">
                                        ...
                                        <div class="note-card-item d-flex justify-content-between align-items-center py-1">
                                            <a href="{{ route('admin.panel.user-subscriptions.edit', $subscription->id) }}" class="btn btn-gradient-primary btn-sm rounded-pill px-3">
                                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                    <path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" />
                                                </svg>
                                            </a>
                ...
                Livewire.on('confirm-delete', (event) => {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            Livewire.dispatch('deleteSubscriptionConfirmed', { id: event.id });
                        }
                    });
                });
