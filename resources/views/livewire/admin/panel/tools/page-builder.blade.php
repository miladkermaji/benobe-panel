      ...
      <div class="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <button wire:click="setPreviewMode('desktop')"
            class="btn btn-glass text-sm py-1 px-3 rounded-full">دسکتاپ</button>
          <button wire:click="setPreviewMode('tablet')"
            class="btn btn-glass text-sm py-1 px-3 rounded-full">تبلت</button>
          <button wire:click="setPreviewMode('mobile')"
            class="btn btn-glass text-sm py-1 px-3 rounded-full">موبایل</button>
          <button wire:click="toggleLivePreview"
            class="btn btn-gradient-primary text-sm py-1 px-3 rounded-full">پیش‌نمایش
            زنده</button>
        </div>
      </div>
    </div>
                  ...
                  <div class="flex items-center p-3">
                    <label class="flex items-center gap-2 text-sm text-gray-700">
                      <input type="checkbox" wire:model="isActive"
                        class="form-check-input h-4 w-4 text-indigo-600 cursor-pointer" id="isActiveNew">
                      <span>فعال</span>
                    </label>
                  </div>
              ...
              @foreach (['text', 'image', 'button', 'video', 'form'] as $type)
                <button wire:click="addElement('{{ $type }}')"
                  ...
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    stroke-width="2" class="hover:rotate-6 transition-transform duration-200">
                    @if ($type === 'text')
              ...
              <div class="space-y-3">
                ...
                @if (in_array($selectedElement->type, ['text', 'button']))
                  <div>
                    ...
                    <input type="color" wire:model="elementSettings.color"
                ...
                <div>
                  ...
                  <input type="text" wire:model="elementSettings.border"
                    class="input-shiny p-2 rounded-lg w-full text-sm" placeholder="1px solid #000">
                </div>
                <div>
                  <label class="block text-sm font-medium text-gray-700">شفافیت (Opacity)</label>
                  <input type="range" wire:model="elementSettings.opacity" min="0" max="1"
                ...
                <button wire:click="updateElement"
                  class="btn btn-gradient-success w-full py-2 rounded-lg text-white text-sm mt-3">ذخیره
                  تغییرات</button>
              </div>
            </div>
          @endif
        ...
        <div class="flex-1">
          @if ($selectedPage)
            <div class="card p-4 rounded-xl shadow-lg bg-white">
              <div class="flex justify-between items-center mb-4 flex-wrap gap-3">
                ...
                <div class="flex items-center gap-2 flex-wrap">
                  ...
                  <button wire:click="saveAsTemplate"
                    class="btn btn-gradient-success py-2 px-4 rounded-lg text-white text-sm">ذخیره به‌عنوان
                    قالب</button>
              ...
              <!-- پیش‌نمایش -->
              <!-- پیش‌نمایش -->
              <div wire:sortable="updateElementOrder"
                class="sortable p-4 rounded-xl border-2 border-dashed border-gray-300"
                  ...
                  <div wire:sortable.item="{{ $element->id }}"
                    ...
                    <div class="flex justify-between items-center">
                      ...
                      @elseif ($element->type === 'button')
                        <button
                          class="btn my-btn-primary py-1 px-3 rounded-lg text-sm">{{ $element->content }}</button>
    ...
    const observer = new MutationObserver((mutations, observer) => {
      ...
      if (sortableEl) {
        new Sortable(sortableEl, {
          ...
          onEnd: (event) => {
            const items = Array.from(sortableEl.querySelectorAll('[wire\\:sortable\\.item]')).map((item,
              index) => ({
