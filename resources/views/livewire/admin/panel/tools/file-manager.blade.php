<div class="container-fluid py-3" wire:init="loadItems">
  <!-- هدر -->
      ...
      <div class="row g-3">
        <div class="col-md-3">
          <div class="input-group">
            ...
            <button wire:click="createFolder" wire:loading.attr="disabled" wire:loading.class="opacity-50"
        ...
        <div class="col-md-3">
          <select wire:model.live="filter" class="form-select rounded-lg shadow-sm">
            <option value="all">همه</option>
            <option value="images">تصاویر</option>
            <option value="text">فایل‌های متنی</option>
            <option value="folders">پوشه‌ها</option>
          </select>
        </div>
        <div class="col-md-3">
  ...
  <div class="container px-0">
    @if (!$ready)
      <div class="text-center py-5">
        ...
        <p class="mt-3 text-gray-600">در حال بارگذاری فایل‌ها...</p>
      </div>
    @else
      <div class="row g-3">
        @if ($currentPath)
            ...
            <div class="card comment-card border-0 rounded-xl shadow-sm bg-gradient-card h-100">
              <div class="card-body p-3 d-flex flex-column align-items-center  text-center">
                @if ($item['type'] === 'folder')
                  ...
                  @else
                    <h6 class="fw-medium text-gray-700 text-ellipsis" style="max-width: 100%;">{{ $item['name'] }}
                    </h6>
                    <div class="d-flex gap-2 mt-2">
    ...
    <div class="fixed inset-0 bg-black bg-opacity-80 d-flex align-items-center justify-content-center z-50">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-3xl p-4">
        ...
        <div class="d-flex gap-3 mt-3 justify-content-end">
          <button wire:click="saveFile" wire:loading.attr="disabled" wire:loading.class="opacity-50"
            class="btn btn-gradient-success px-3 rounded-lg">
            <span wire:loading.remove wire:target="saveFile">ذخیره</span>
            <span wire:loading wire:target="saveFile">
              <i class="fas fa-spinner fa-spin me-1"></i> در حال ذخیره...
            </span>
          </button>
