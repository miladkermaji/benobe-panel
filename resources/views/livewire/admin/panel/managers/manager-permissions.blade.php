      ...
      <div x-show="mobileSearchOpen" x-transition:enter="transition ease-out duration-300"
        ...
        <div class="d-flex flex-column gap-2">
          <div class="search-box position-relative">
            <input type="text" wire:model.live="search" class="form-control ps-5" placeholder="جستجو در مدیران...">
            ...
            <tbody x-data="{ open: false }">
              <tr style="background: #f5f7fa;  cursor:pointer;" @click="open = !open">
                <td class="py-2 px-3 fw-bold text-primary" style="font-size: 1.05rem;">
                  ...
                  {{ $manager->first_name }} {{ $manager->last_name }} @if ($manager->mobile)
                    ({{ $manager->mobile }})
                  @endif
                </td>
                        ...
                        <div class="d-flex align-items-center mb-2">
                          <label class="custom-checkbox flex items-center gap-2">
                            <input type="checkbox" class="custom-checkbox-input parent-checkbox form-check-input"
                              data-manager-id="{{ $manager->id }}" data-permission-key="{{ $permissionKey }}"
                              id="parent-{{ $manager->id }}-{{ $permissionKey }}"
                              wire:change="updatePermissions({{ $manager->id }}, $event.target.checked ? {{ json_encode(array_merge($savedPermissions, [$permissionKey], array_keys($permissionData['routes'] ?? []))) }} : {{ json_encode(array_filter($savedPermissions, fn($p) => $p !== $permissionKey && !in_array($p, array_keys($permissionData['routes'] ?? [])))) }})"
                              ...
                              <div class="d-flex align-items-center mb-2">
                                <label class="custom-checkbox flex items-center gap-2">
                                  <input type="checkbox" class="custom-checkbox-input child-checkbox form-check-input"
                                    data-parent-id="parent-{{ $manager->id }}-{{ $permissionKey }}"
                                    id="child-{{ $manager->id }}-{{ $routeKey }}"
                                    wire:change="updatePermissions({{ $manager->id }}, $event.target.checked ? {{ json_encode(array_merge($savedPermissions, [$routeKey])) }} : {{ json_encode(array_filter($savedPermissions, fn($p) => $p !== $routeKey)) }})"
