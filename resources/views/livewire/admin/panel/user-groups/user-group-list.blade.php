<div class="doctor-notes-container">
    <div class="container py-2 mt-3" dir="rtl" wire:init="loadUserGroups">
        <div class="glass-header text-white p-2 rounded-2 mb-4 shadow-lg">
            <div class="d-flex flex-column flex-md-row align-items-start align-items-md-center gap-3 w-100">
                <div class="d-flex flex-column flex-md-row gap-2 w-100 align-items-center justify-content-between">
                    <div class="d-flex align-items-center gap-3">
                        <h1 class="m-0 h4 font-thin text-nowrap mb-3 mb-md-0">مدیریت گروه‌های کاربری</h1>
                    </div>
                    <div class="d-flex flex-column flex-md-row align-items-stretch align-items-md-center gap-2 w-100">
                        <div class="d-flex gap-2 flex-shrink-0 justify-content-center w-100 flex-column flex-md-row">
                            ...
                            <a href="{{ route('admin.panel.user-groups.create') }}"
            ...
            <div class="card shadow-sm rounded-2">
                <div class="card-body p-0">
                    ...
                    <div class="group-actions p-2 border-bottom" x-data="{ show: false }"
                         x-show="$wire.selectedUserGroups.length > 0 || $wire.applyToAllFiltered">
                        <div class="d-flex align-items-center gap-2 justify-content-end">
                            <select class="form-select form-select-sm" style="max-width: 200px;"
                                    wire:model="groupAction">
                            ...
                            <button class="btn btn-sm btn-primary" wire:click="executeGroupAction"
                    ...
                    <div class="table-responsive text-nowrap d-none d-md-block">
                        <table class="table table-hover w-100 m-0">
                            ...
                            <tbody>
                            @if ($readyToLoad)
                                @forelse ($userGroups as $index => $userGroup)
                                    <tr class="align-middle">
                                        <td class="text-center">
                                            <div class="d-flex justify-content-center align-items-center">
                                                <input type="checkbox" wire:model.live="selectedUserGroups"
                                                       value="{{ $userGroup->id }}"
                                                       class="form-check-input m-0 align-middle">
                                            </div>
                                        </td>
                                        <td class="text-center">{{ $userGroups->firstItem() + $index }}</td>
                                        <td class="align-middle">{{ $userGroup->name }}</td>
                                        <td class="align-middle">{{ $userGroup->description ?? '-' }}</td>
                                        <td class="text-center align-middle">
                                            <button wire:click="toggleStatus({{ $userGroup->id }})"
                                                    class="badge {{ $userGroup->is_active ? 'bg-label-success' : 'bg-label-danger' }} border-0 cursor-pointer">
                                                {{ $userGroup->is_active ? 'فعال' : 'غیرفعال' }}
                                            </button>
                                        </td>
                                        <td class="text-center align-middle">
                                            <div class="d-flex justify-content-center gap-2">
                                                <a href="{{ route('admin.panel.user-groups.edit', $userGroup->id) }}"
                                                ...
                                                <button wire:click="confirmDelete({{ $userGroup->id }})"
                            ...
                            @forelse ($userGroups as $index => $userGroup)
                                <div class="note-card mb-2" x-data="{ open: false }">
                                    <div class="note-card-header d-flex justify-content-between align-items-center px-2 py-2"
                                        ...
                                        <svg :class="{ 'rotate-180': open }" width="20" height="20"
                                             viewBox="0 0 24 24"
                                             fill="none" stroke="currentColor" stroke-width="2"
                                    ...
                                    <div class="note-card-body px-2 py-2" x-show="open" x-transition>
                                        <div class="note-card-item d-flex justify-content-between align-items-center py-1">
                                            ...
                                            <span class="note-card-value">{{ $userGroup->name }}</span>
                                        </div>
                                        <div class="note-card-item d-flex justify-content-between align-items-center py-1">
                                            <span class="note-card-label">توضیحات:</span>
                                            <span class="note-card-value">{{ $userGroup->description ?? '-' }}</span>
                                        </div>
                                        <div class="note-card-item d-flex justify-content-between align-items-center py-1">
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-4 px-4 flex-wrap gap-3">
                        <div class="text-muted">نمایش {{ $userGroups ? $userGroups->firstItem() : 0 }} تا
                            {{ $userGroups ? $userGroups->lastItem() : 0 }} از {{ $userGroups ? $userGroups->total() : 0 }} ردیف
                        </div>
                        @if ($userGroups && $userGroups->hasPages())
                            <div class="pagination-container">
                                {{ $userGroups->onEachSide(1)->links('livewire::bootstrap') }}
                            </div>
                        @endif
                    </div>
                </div>
                ...
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    Livewire.on('confirm-delete', (event) => {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                Livewire.dispatch('deleteUserGroupConfirmed', {
                                    id: event.id
                                });
                            }
                        });
                    ...
                    Livewire.on('confirm-delete-selected', function (data) {
                        let text = data.allFiltered ?
                            'آیا از حذف همه گروه‌های کاربری فیلترشده مطمئن هستید؟ این عملیات غیرقابل بازگشت است.' :
                            'آیا از حذف گروه‌های کاربری انتخاب شده مطمئن هستید؟ این عملیات غیرقابل بازگشت است.';
                        Swal.fire({
