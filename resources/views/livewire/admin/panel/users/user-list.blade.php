<div class="container-fluid py-2" dir="rtl" wire:init="loadUsers">
    <div class="glass-header text-white p-3 rounded-3 mb-5 shadow-lg d-flex justify-content-between align-items-center flex-wrap gap-3">
        ...
        <div class="input-group flex-grow-1 position-relative" style="max-width: 400px;">
            <input type="text" class="form-control border-0 shadow-none bg-white text-dark ps-5 rounded-3"
                wire:model.live="search" placeholder="جستجو در کاربران..." style="padding-right: 23px">
                                    ...
                                    <tr>
                                        <td class="text-center align-middle">
                                            <input type="checkbox" wire:model.live="selectedUsers"
                                                value="{{ $user->id }}" class="form-check-input m-0">
                                        </td>
                                        <td class="text-center align-middle">{{ $users->firstItem() + $index }}</td>
                                        <td class="text-center align-middle">
                                            <img src="{{ $user->profile_photo_url }}" class="rounded-circle" style="width: 40px; height: 40px; object-fit: cover;" alt="پروفایل">
                                        </td>
                                        ...
                                        <td class="text-center align-middle">
                                            <button wire:click="toggleStatus({{ $user->id }})"
                                                class="badge {{ $user->activation ? 'bg-label-success' : 'bg-label-danger' }} border-0 cursor-pointer">
                                                {{ $user->activation ? 'فعال' : 'غیرفعال' }}
                                            </button>
                                        </td>
                                        <td class="text-center align-middle">
    ...
    <link rel="stylesheet" href="{{ asset('admin-assets/panel/css/users/users.css') }}">
    <script>
