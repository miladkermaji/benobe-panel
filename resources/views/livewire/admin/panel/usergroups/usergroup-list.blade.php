<div class="container-fluid py-2" dir="rtl" wire:init="loadusergroups">
  <div class="glass-header text-white p-3 rounded-3 mb-5 shadow-lg d-flex justify-content-between align-items-center flex-wrap gap-3">
    <h1 class="m-0 h3 font-thin flex-grow-1" style="min-width: 200px;">مدیریت گروه کاربری</h1>
    <div class="input-group flex-grow-1 position-relative" style="max-width: 400px;">
      <input type="text" class="form-control border-0 shadow-none bg-white text-dark ps-5 rounded-3" wire:model.live="search" placeholder="جستجو در گروه کاربری..." style="padding-right: 23px">
    ...
    <div class="d-flex gap-2 flex-shrink-0 flex-wrap justify-content-center mt-md-2 buttons-container">
      <a href="{{ route('admin.panel.usergroups.create') }}" class="btn btn-gradient-success rounded-pill px-4 d-flex align-items-center gap-2">
        <svg style="transform: rotate(180deg)" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <path d="M12 5v14M5 12h14" />
        </svg>
        <span>افزودن گروه کاربری</span>
      </a>
      <button wire:click="deleteSelected" class="btn btn-gradient-danger rounded-pill px-4 d-flex align-items-center gap-2" @if (empty($selectedusergroups)) disabled @endif>
        <svg style="transform: rotate(180deg)" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div class="card shadow-sm">
      <div class="card-body p-0">
        <div class="table-responsive text-nowrap">
          <table class="table table-bordered table-hover w-100 m-0">
            ...
            <tbody>
                ...
                @forelse ($usergroups as $index => $item)
                  <tr>
                    <td class="text-center align-middle">
                      <input type="checkbox" wire:model.live="selectedusergroups" value="{{ $item->id }}" class="form-check-input m-0">
                    </td>
                    ...
                    <td class="text-center align-middle">
                      <div class="d-flex justify-content-center gap-2">
                        <a href="{{ route('admin.panel.usergroups.edit', $item->id) }}" class="btn btn-gradient-success rounded-pill px-3">
              ...
              @else
                <tr>
                  <td colspan="6" class="text-center py-5">در حال بارگذاری گروه کاربری...</td>
                </tr>
              @endif
            </tbody>
          </table>
        ...
        <div class="d-flex justify-content-between align-items-center mt-4 px-4 flex-wrap gap-3">
          <div class="text-muted">نمایش {{ $usergroups ? $usergroups->firstItem() : 0 }} تا {{ $usergroups ? $usergroups->lastItem() : 0 }} از {{ $usergroups ? $usergroups->total() : 0 }} ردیف</div>
          @if ($usergroups && $usergroups->hasPages())
            {{ $usergroups->links('livewire::bootstrap') }}
          @endif
        </div>
      </div>
    </div>
      ...
      Livewire.on('confirm-delete', (event) => {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            Livewire.dispatch('deleteUserGroupConfirmed', { id: event.id });
          }
        });
      });
    });
