<div class="container-fluid py-4" dir="rtl" wire:init="loadDoctorComments">
  <div class="glass-header p-4 rounded-xl mb-6 shadow-lg d-flex justify-content-between align-items-center flex-wrap gap-4">
      <h1 class="m-0 h3 font-light flex-grow-1" style="min-width: 200px; color: var(--text-primary);">مدیریت نظرات پزشکان</h1>
      ...
      <div class="d-flex gap-3 flex-shrink-0 flex-wrap justify-content-center mt-md-0 buttons-container">
          ...
          <div class="dropdown">
              ...
              <ul class="dropdown-menu rounded-xl shadow-lg bg-background-card border-0 mt-2">
                  <li><a class="dropdown-item px-4 py-2 hover:bg-background-light" wire:click="toggleSelectedStatus">تغییر وضعیت</a></li>
                  <li><a class="dropdown-item px-4 py-2 hover:bg-background-light" wire:click="deleteSelected">حذف انتخاب‌شده‌ها</a></li>
              </ul>
          </div>
      </div>
  </div>
                      ...
                      <div class="doctor-toggle border-bottom transition-all duration-300 hover:bg-background-light">
                          <div class="d-flex justify-content-between align-items-center p-4 cursor-pointer"
                              ...
                              <div class="d-flex align-items-center gap-3">
                                  ...
                                  <span class="fw-bold text-text-primary text-lg">{{ $doctor->first_name . ' ' . $doctor->last_name }}</span>
                                  <span class="badge-comment bg-gradient-primary text-white font-medium px-3 py-1 rounded-full shadow-sm hover:shadow-md transition-all duration-300">{{ $doctor->comments->total() }} نظر</span>
                              </div>
                          ...
                          @if (in_array($doctor->id, $expandedDoctors))
                              <div class="table-responsive text-nowrap p-4 bg-background-light">
                                              ...
                                              <tr class="hover:bg-background-light transition-colors duration-200">
                                                  ...
                                                  <td class="text-center align-middle">
                                                      <label class="toggle-switch">
                                                          <input type="checkbox" wire:model.live="commentStatus.{{ $comment->id }}" wire:change="toggleStatus({{ $comment->id }})" class="toggle-input">
                                                  ...
                                                  <td class="text-center align-middle">
                                                      <div class="d-flex justify-content-center gap-2">
                                                          <a href="{{ route('admin.panel.doctor-comments.edit', $comment->id) }}"
                                                              ...
                                                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                                  <path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7M18.5 2.5a2.121 2.121 0 013 3L12 12 8 13 9 9l9.5-9.5z" />
                                                              </svg>
                                                          </a>
                                                          <button wire:click="toggleReply({{ $comment->id }})"
                                                              ...
                                                              title="پاسخ">
                                                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                                  <path d="M21 11.5a8.38 8.38 0 01-11.9 7.6L3 21l1.9-5.7a8.38 8.38 0 017.6-11.9A8.38 8.38 0 0121 11.5z" />
                                                              </svg>
                                                          </button>
                                                      ...
                                                      @if ($replyingTo === $comment->id)
                                                          <div class="mt-3 p-3 bg-background-light rounded-xl shadow-inner">
                                                              ...
                                                              <button wire:click="saveReply({{ $comment->id }})"
                                                                  class="btn btn-gradient-success rounded-full px-4 py-1.5 shadow-sm hover:shadow-md transition-all duration-300">ارسال پاسخ</button>
                                                          </div>
                                                      @endif
                                                  </td>
                                              </tr>
                                              ...
                                              <div class="card-body p-3">
                                                  <div class="d-flex justify-content-between align-items-center mb-2">
                                                      <div class="d-flex align-items-center gap-2">
                                                          <input type="checkbox" wire:model.live="selectedDoctorComments" value="{{ $comment->id }}"
                                                              class="form-check-input m-0 shadow-sm">
                                                          <span class="text-text-secondary font-medium text-sm">#{{ $doctor->comments->firstItem() + $index }}</span>
                                                      </div>
                                                      <label class="toggle-switch">
                                                          <input type="checkbox" wire:model.live="commentStatus.{{ $comment->id }}" wire:change="toggleStatus({{ $comment->id }})" class="toggle-input">
                                                          <span class="toggle-slider"></span>
                                                  ...
                                                  <div class="mb-2">
                                                      <p class="text-text-primary mb-1 text-sm"><strong>نام کاربر:</strong> {{ $comment->user_name }}</p>
                                                      <p class="text-text-secondary mb-1 text-sm"><strong>شماره تماس:</strong> {{ $comment->user_phone ?? 'ثبت نشده' }}</p>
                                                      <p class="text-text-primary mb-1 text-sm"><strong>نظر:</strong> {{ Str::limit($comment->comment, 50) }}</p>
                                                      @if ($comment->reply)
                                  ...
                                  @if ($doctor->comments->hasPages())
                                      <div class="p-4 d-flex justify-content-between align-items-center flex-wrap gap-4 bg-background-light">
          ...
          Livewire.on('confirm-delete', (event) => {
              Swal.fire({
                  ...
                  if (result.isConfirmed) {
                      Livewire.dispatch('deleteDoctorCommentConfirmed', { id: event.id });
                  }
              });
          });
      });
