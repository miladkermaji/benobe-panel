                  ...
                  <div class="form-group position-relative">
                    <x-custom-tooltip
                      ...
                      <button class="btn btn-light btn-sm emergency-slot-btn"
                        data-day="{{ $workSchedule['data']['day'] }}"
                        wire:click="$dispatch('openXModal', { id: 'emergencyModal' })"
                        data-index="{{ $index }}"
                        @if(!$isEditable || empty($slot['start']) || empty($slot['end']) || empty($slot['max_appointments'])) disabled @endif>
        ...
        <div class="alert alert-warning text-center">
          ...
          <div class="mt-3">
            <button class="btn btn-primary w-100 h-50" wire:click="addSlot"
              @if($isProcessing || !$isEditable) disabled @endif>
              افزودن بازه زمانی
            </button>
          </div>
        </div>
    ...
    document.addEventListener('livewire:initialized', () => {
      ...
      $(document).ready(function() {
        ...
        $(document).on('click', '#saveSchedule', function() {
          ...
          if (!isEditable) {
            toastr.error('حالت ویرایش غیرفعال است.');
            return;
          }
          ...
          try {
            ...
            @this.call('saveSchedule', startTime, endTime, selectedDays).then(() => {
              toggleButtonLoading($button, false);
