<div wire:key="special-workhours-{{ $selectedDate ?? 'default' }}">
        ...
        <div class="workhours-wrapper-content p-3">
            @if ($hasWorkHoursMessage)
                <div class="alert alert-info" role="alert">
                    <p class="fw-bold text-center">شما از قبل برای این روز ساعات کاری تعریف کرده‌اید. در صورت تمایل می‌توانید آن را ویرایش کنید</p>
                </div>
            @endif

            @if ($workSchedule['status'] && !empty($workSchedule['data']['work_hours']))
                <div class="border-333 p-3 mt-3 border-radius-11">
                    <h6>ساعات کاری - {{ $selectedDate ? \Morilog\Jalali\Jalalian::fromCarbon(\Carbon\Carbon::parse($selectedDate))->format('d F Y') : '' }}</h6>
                    <div class="mt-4">
                        @foreach ($workSchedule['data']['work_hours'] as $index => $slot)
                            <div class="form-row d-flex w-100 p-3 bg-active-slot border-radius-11" data-slot-id="{{ $index }}" wire:key="slot-{{ $index }}">
                                <div class="d-flex justify-content-start align-items-center gap-4">
                                    ...
                                    <div class="form-group position-relative">
                                        <label class="label-top-input-special-takhasos" for="patients-{{ $index }}">تعداد نوبت</label>
                                        <input 
                                            type="text" 
                                    ...
                                    <div class="form-group position-relative">
                                        ...
                                        >
                                            <button 
                                                class="btn btn-light btn-sm emergency-slot-btn"
                                    ...
                                    <div class="form-group position-relative">
                                        <x-custom-tooltip title="حذف برنامه کاری" placement="top">
                                            <button 
                                                class="btn btn-light btn-sm remove-row-btn" 
                    ...
                    <div class="add-new-row mt-3">
                        <button 
                            class="add-row-btn btn btn-sm btn-light" 
                            data-tooltip="true" 
    ...
    <div>
        <x-custom-modal 
            id="CalculatorModal" 
            title="انتخاب تعداد نوبت یا زمان ویزیت" 
            size="sm" 
            :show="false"
        ...
        >
                    ...
                    <div class="d-flex align-items-center w-100">
                        <div class="d-flex align-items-center">
                            <input 
                                ...
                                wire:model.live="calculator.calculation_mode" 
                                value="count"
                            >
                        ...
                        <div class="input-group position-relative mx-2">
                            ...
                            <input 
                                type="number" 
                                class="form-control text-center h-50 rounded-0 border-radius-0"
                                id="appointment-count" 
                                wire:model.live.debounce.300ms="calculator.appointment_count"
                                wire:focus="setCalculationMode('count')" 
                                style="height: 50px;"
                            >
                    ...
                    <div class="d-flex align-items-center mt-4 w-100">
                        ...
                        <div class="input-group position-relative mx-2">
                            ...
                            <input 
                                ...
                                wire:focus="setCalculationMode('time')"
                                style="height: 50px;"
                            >
            ...
            <div class="w-100 d-flex justify-content-end p-1 gap-4 mt-3">
                <button 
                    type="button" 
                    class="btn my-btn-primary w-100 d-flex justify-content-center align-items-center"
                    wire:click="saveCalculator" 
    ...
    <div>
        <x-custom-modal 
            id="emergencyModal" 
            title="انتخاب زمان‌های اورژانسی" 
            size="md" 
            :show="$isEmergencyModalOpen"
                            ...
                            @foreach ($emergencyTimes['possible'] as $time)
                                <div class="form-check">
                                    <input 
                                        type="checkbox" 
                                        class="form-check-input"
                                        id="emergency-time-{{ $time }}"
                                        wire:model.live="selectedEmergencyTimes.{{ $time }}"
                                        value="{{ $time }}"
                                    >
                        ...
                        <div class="day-schdule-wrapper">
                            <div class="day-checkbox">
                                <x-my-check-box 
                                    ...
                                    is-checked="{{ $selectAllScheduleModal }}"
                                    day="انتخاب همه" 
                    ...
                    <div class="timepicker-save-section">
                        <div class="form-group position-relative timepicker-ui">
                            ...
                            <input 
                                data-timepicker 
                                type="text" 
                                class="form-control timepicker-ui-input text-center fw-bold" 
                                id="schedule-start"
                                value="{{ $workSchedule['data']['work_hours'][$scheduleModalIndex]['start'] ?? '00:00' }}"
                            >
                        </div>
                        <div class="form-group position-relative timepicker-ui">
                            ...
                            <input 
                                ...
                                class="form-control timepicker-ui-input text-center fw-bold" 
                                id="schedule-end"
                                value="{{ $workSchedule['data']['work_hours'][$scheduleModalIndex]['end'] ?? '23:59' }}"
                            >
                        </div>
                        <button 
                            type="button" 
                            class="btn my-btn-primary d-flex justify-content-center align-items-center"
                            id="saveSchedule"
                            @if ($isProcessing) disabled @endif
                        >
                            <span class="button_text">ذخیره تغییرات</span>
                                ...
                                @if (!empty($filteredSettings))
                                    @foreach ($filteredSettings as $index => $setting)
                                        <div class="schedule-setting-item"
                                            ...
                                            <button 
                                                class="btn btn-light delete-schedule-setting" 
                                                data-day="{{ $scheduleModalDay }}"
                                                data-index="{{ $index }}"
    ...
    <script>
        ...
        document.addEventListener('livewire:initialized', () => {
            ...
            $(document).on('click', '#saveSchedule', function() {
                ...
                try {
                    ...
                    @this.call('saveSchedule', startTime, endTime).then(() => {
                        ...
                        Livewire.dispatch('refresh-schedule-settings');
                    }).catch((error) => {
                        toggleButtonLoading($button, false);
            ...
            function initializeCalculatorModal(params) {
                setTimeout(() => {
                    ...
                    $appointmentCount.off('input').on('input', function() {
                        ...
                        console.log('Appointment count input:', { count });
                    ...
                    $timeCount.off('input').on('input', function() {
                        ...
                        console.log('Time per appointment input:', { time });
            ...
            Livewire.on('openXModal', ({ id }) => {
                console.log('Opening modal:', id);
                setTimeout(() => {
                    window.openXModal(id);
                    Livewire.dispatch('refresh-timepicker');
                }, 100);
            });

            Livewire.on('closeXModal', ({ id }) => {
