<tr>
    ...
    <td class="text-center align-middle">
        <button wire:click="toggleStatus({{ $service->id }})" class="badge {{ $service->status ? 'bg-label-success' : 'bg-label-danger' }} border-0 cursor-pointer">
            {{ $service->status ? 'فعال' : 'غیرفعال' }}
        </button>
    </td>
    <td class="text-center align-middle">
        <div class="d-flex justify-content-center gap-2">
            <a href="{{ route('dr.panel.doctor-services.edit', $service->id) }}" class="btn btn-gradient-success rounded-pill px-3">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <table class="table table-bordered w-100 m-0">
                <tbody>
                    @foreach($service->children as $child)
                        @include('livewire.dr.panel.doctorservices.doctor-service-dropdown', ['service' => $child, 'level' => $level + 1, 'index' => $index . '.' . ($loop->index + 1)])
                    @endforeach
                </tbody>
            </table>
        </td>
