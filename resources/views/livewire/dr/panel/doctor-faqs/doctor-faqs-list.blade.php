<div class="container-fluid py-2" dir="rtl">
  <div
    ...
    <h1 class="m-0 h3 font-thin flex-grow-1" style="min-width: 200px;">مدیریت سوالات متداول</h1>
    <div class="input-group flex-grow-1 position-relative" style="max-width: 400px;">
      <input type="text" class="form-control border-0 shadow-none bg-white text-dark ps-5 rounded-3"
        wire:model.live.debounce.500ms="search" placeholder="جستجو در سوالات متداول...">
    ...
    <div class="d-flex gap-2 flex-shrink-0 flex-wrap justify-content-center mt-md-2 buttons-container">
      <a href="{{ route('dr.panel.doctor-faqs.create') }}"
      ...
      <button wire:click="deleteSelected"
        ...
        @if (empty($selectedFaqs)) disabled @endif>
        <svg style="transform: rotate(180deg)" width="16" height="16" viewBox="0 0 24 24" fill="none"
          stroke="currentColor" stroke-width="2">
    ...
    <div class="card shadow-sm">
      <div class="card-body p-0">
        <div class="table-responsive text-nowrap">
          <table class="table  w-100 m-0">
            ...
            <tbody>
              @if ($readyToLoad)
                @forelse ($faqs as $index => $faq)
                  <tr>
                    ...
                    <td class="align-middle">{{ Str::limit($faq->answer, 100) }}</td>
                    <td class="text-center align-middle">
                      <button wire:click="toggleStatus({{ $faq->id }})"
                        class="badge {{ $faq->is_active ? 'bg-label-success' : 'bg-label-danger' }} border-0 cursor-pointer">
                        {{ $faq->is_active ? 'فعال' : 'غیرفعال' }}
                      </button>
                    </td>
                    <td class="text-center align-middle">
                      <div class="d-flex justify-content-center gap-2">
                        <a href="{{ route('dr.panel.doctor-faqs.edit', $faq->id) }}"
              ...
              @else
                <tr>
                  <td colspan="6" class="text-center py-5">در حال بارگذاری سوالات متداول...</td>
                </tr>
              @endif
            </tbody>
          </table>
        ...
        <div class="d-flex justify-content-between align-items-center mt-4 px-4 flex-wrap gap-3">
          <div class="text-muted">نمایش {{ $faqs ? $faqs->firstItem() : 0 }} تا
            {{ $faqs ? $faqs->lastItem() : 0 }} از {{ $faqs ? $faqs->total() : 0 }} ردیف
          </div>
      ...
      window.Livewire.on('confirm-delete', ({
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            window.Livewire.dispatch('deleteFaqConfirmed', {
              id
            });
          }
        });
