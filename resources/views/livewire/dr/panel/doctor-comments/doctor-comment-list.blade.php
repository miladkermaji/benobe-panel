<div class="container-fluid py-2" dir="rtl">
  <div class="header-custom p-2 rounded-lg mb-3 shadow-sm d-flex flex-column flex-md-row justify-content-between align-items-center gap-2"
    style="background: var(--background-light); border: 1px solid var(--border-neutral);">
    <h1 class="m-0 h5 font-medium flex-grow-1 text-center text-md-start" style="color: var(--text-primary);">مدیریت نظرات</h1>
    <div class="input-group flex-grow-1" style="max-width: 100%; width: 100%; max-width: 350px;">
      <input type="text" class="form-control border-0 rounded-lg h-40" wire:model.live="search"
        placeholder="جستجو..." style="background: var(--background-card); color: var(--text-primary); font-size: 0.85rem;">
    </div>
    <div class="d-flex gap-2 flex-shrink-0">
      <button wire:click="toggleSelectedStatus" class="btn btn-secondary rounded-lg px-3 py-1 d-flex align-items-center gap-1"
        ...
        <svg class="d-md-none" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div class="card shadow-sm rounded-lg" style="background: var(--background-card);">
      <div class="card-body p-0">
        @if ($readyToLoad)
              ...
              <tbody>
                @forelse ($comments as $index => $comment)
                  <tr>
                    ...
                    <td class="text-center" style="color: var(--text-secondary);">{{ $comments->firstItem() + $index }}</td>
                    ...
                    <td class="text-center">
                      <div class="d-flex justify-content-center gap-2">
                        <button wire:click="toggleReply({{ $comment->id }})" class="btn btn-primary rounded px-2 py-1" title="پاسخ">
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M21 11.5a8.38 8.38 0 01-11.9 7.6L3 21l1.9-5.7a8.38 8.38 0 017.6-11.9A8.38 8.38 0 0121 11.5z" />
                          </svg>
                        </button>
                        <button wire:click="confirmDelete({{ $comment->id }})" class="btn btn-danger rounded px-2 py-1">
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2" />
                          </svg>
                        </button>
                    ...
                    <tr>
                      <td colspan="7" class="p-2">
                        <div class="rounded-lg shadow-inner" style="background: var(--background-light);">
                          ...
                          <button wire:click="saveReply({{ $comment->id }})" class="btn btn-secondary rounded-lg px-3 py-1">ارسال</button>
                        </div>
                      </td>
                    </tr>
                  @endif
            ...
            @forelse ($comments as $index => $comment)
              <div class="card mb-2 p-2 shadow-sm rounded-lg">
                <div class="d-flex align-items-center gap-2">
                  <input type="checkbox" wire:model.live="selectedDoctorComments" value="{{ $comment->id }}" class="form-check-input">
                  <div class="flex-grow-1">
                    <div class="d-flex justify-content-between">
                      <span class="font-medium" style="color: var(--text-primary);">{{ $comment->doctor->full_name }}</span>
          ...
          @endif
        @else
          <div class="text-center py-3" style="color: var(--text-secondary); font-size: 0.85rem;">در حال بارگذاری...</div>
        @endif
      </div>
    </div>
  </div>
