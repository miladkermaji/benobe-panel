<div class="service-container" x-data="{ mobileSearchOpen: false }" wire:key="services-{{ $refreshKey ?? '' }}">
          ...
          <div x-show="mobileSearchOpen" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="d-flex flex-column flex-md-row align-items-stretch align-items-md-center gap-2">
              <div class="d-flex gap-2 flex-shrink-0 justify-content-center">
                ...
                <a href="{{ route('mc.panel.services.create') }}"
    ...
    <div class="container-fluid px-0">
      <div class="card shadow-sm rounded-2">
        <div class="card-body p-0">
                ...
                @if ($readyToLoad)
                  @forelse($services as $index => $service)
                    <tr>
                      ...
                      <td class="align-middle">
                        <span class="text-muted">{{ $service->description ?: 'بدون توضیحات' }}</span>
                      </td>
                      <td class="text-center align-middle">
                      ...
                      <td class="text-center align-middle">
                        <div class="btn-group" role="group">
                          <button type="button" class="btn btn-sm btn-outline-primary"
                            wire:click="confirmToggleStatus({{ $service->id }})"
                            title="{{ $service->status ? 'غیرفعال کردن' : 'فعال کردن' }}">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
                              stroke="currentColor" stroke-width="2">
                              <path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                          </button>
                          <a href="{{ route('mc.panel.services.edit', $service->id) }}"
                          ...
                          <button type="button" class="btn btn-sm btn-outline-danger"
                            wire:click="confirmDelete({{ $service->id }})" title="حذف">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
                              stroke="currentColor" stroke-width="2">
                              <path
                    ...
                    <tr>
                      <td colspan="6" class="text-center py-4">
                        <div class="d-flex flex-column align-items-center">
                          ...
                          <p class="text-muted mt-2 mb-0">هیچ خدمتی یافت نشد</p>
                        </div>
                      </td>
                    </tr>
                  @endforelse
                ...
                <div class="note-card mb-3" x-data="{ open: false }">
                  <div class="note-card-header d-flex justify-content-between align-items-center"
                    ...
                    <div class="d-flex align-items-center gap-2">
                      ...
                      <span class="badge bg-primary-subtle text-primary">{{ $service->name }}</span>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                  ...
                  <div class="note-card-body" x-show="open" x-transition:enter="transition ease-out duration-300"
                    ...
                    <div class="note-card-item">
                      ...
                      <span class="note-card-value">{{ $service->description ?: 'بدون توضیحات' }}</span>
                    </div>
                    <div class="note-card-item">
                      ...
                      <div class="form-check form-switch d-inline-block">
                        <input class="form-check-input" type="checkbox" role="switch"
                          wire:click="confirmToggleStatus({{ $service->id }})"
                          {{ $service->status ? 'checked' : '' }} style="width: 3em; height: 1.5em; margin-top: 0;">
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <!-- Pagination -->
          @if ($readyToLoad && $services->hasPages())
  ...
  <script>
    document.addEventListener('livewire:init', function() {
      ...
      Livewire.on('confirm-delete-selected', (event) => {
        Swal.fire({
          title: 'حذف گروهی خدمات‌ها',
          text: 'آیا مطمئن هستید که می‌خواهید خدمات انتخاب شده را حذف کنید؟',
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#ef4444',
          cancelButtonColor: '#6b7280',
      ...
      Livewire.on('confirm-toggle-status', (event) => {
        Swal.fire({
          title: event.action + ' خدمت',
          text: `آیا مطمئن هستید که می‌خواهید خدمت "${event.name}" را ${event.action} کنید؟`,
          icon: 'question',
          showCancelButton: true,
