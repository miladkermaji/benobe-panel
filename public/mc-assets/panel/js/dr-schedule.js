$(function () {
    ...
    function getHolidayInfo(date) {
        ...
        if (monthHolidays) {
            return (
                monthHolidays.find(
                    (holiday) => parseInt(holiday.day) === day
                ) || null
            );
        }
        return null;
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.manualAppointments.length > 0) {
                    ...
                    response.manualAppointments.forEach(function (appointment) {
                        ...
                        const row = $(`
                                <tr>
                                    ...
                                    <td>${
                                        appointment.title ?? "بدون عنوان"
                                    }</td>
                                    <td>${appointment.max_appointments}</td>
                                    <td>${
                                        appointment.include_holidays === 1
                                            ? "بله"
                                            : "خیر"
                                    }</td>
                                    <td><button class="btn btn-sm btn-light rounded-circle btn-del-appointments" data-id="${
                                        appointment.id
                                    }">
                    ...
                    $("#patterns-list-appointments").off(
                        "click",
                    ...
                    $("#patterns-list-appointments").on(
                        ...
                        function () {
                            ...
                            var deleteUrl = appointmentsDestroy.replace(
                                ":id",
                                appointmentId
                            );
                                    ...
                                    $.ajax({
                                        ...
                                        success: function (response) {
                                            ...
                                            $.ajax({
                                                ...
                                                success: function (
                                                    ...
                                                    var trustDate =
                                                        convertedResponse.gregorianDate.split(
                                                            "-"
                                                        );
                                                    if (trustDate[1] < 10)
                                                        trustDate[1] =
                                        ...
                                        error: function (error) {
                                            ...
                                            if (
                                                ...
                                                errorMessage =
                                                    error.responseJSON.error; // گرفتن پیام خطا از پاسخ سرور
                                            }
                                            Swal.fire({
                                                title: "خطا!",
                                                text: errorMessage,
    ...
    function fetchDisabledDays() {
        $.ajax({
            ...
            success: function (response) {
                if (
                    response.disabled_days &&
                    Array.isArray(response.disabled_days)
                ) {
                    disabled_days = [];
    ...
    $("#datepicker7").datepicker({
        ...
        beforeShowDay: function (date) {
            ...
            let formattedDate = `${persianDay} ${getPersianMonthName(
                persianMonth - 1
            )} ${persianYear}`;
            ...
            const gDate = jalaali.toGregorian(
                parseInt(year),
                month,
                parseInt(day)
            );
            // فرمت YYYY-MM-DD
            const formattedDateNew = `${gDate.gy}-${String(gDate.gm).padStart(
                2,
                "0"
            ...
            if (disabled_daysCount > 0) {
                setTimeout(() => {
                    $("#datepicker7 .ui-datepicker-calendar td").each(
                        function () {
                            const $this = $(this);
        ...
        onSelect: function (dateText, inst) {
            ...
            $("#currentNobatText").html(
                "لیست نوبت های " + currentClickDatePersian
            );
            ...
            if (
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.status === "success") {
                            if (response.wasDisabled == "غیرفعال") {
                                message =
                                    "الگوی نوبت دهی خودکار برای این روز غیرفعال شد";
                                if (
                                    $("#disabledDays").is(":checked") === true
                                ) {
                                    $("#disabledDays").prop("checked", false);
                                }
                            } else {
                    ...
                    error: function (error) {
                        let errorMessage =
                            "مشکلی در بروزرسانی دیتابیس پیش آمده است.";
            ...
            if (holidayInfo) {
                ...
                const popupContent = `
                    <div class="popup-content">
                        <strong>تاریخ: </strong> ${selectedDate.getDate()} ${getPersianMonthName(
                    selectedDate.getMonth()
                )} ${selectedDate.getFullYear()}<br>
                        <strong>مناسبت: </strong> ${holidayInfo.occasion} <br>
                        <strong>تعطیل: </strong> ${
                            holidayInfo.isHoliday ? "true" : "false"
                        }
                    </div>
                `;
