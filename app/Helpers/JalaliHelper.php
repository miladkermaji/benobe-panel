...
class JalaliHelper
{
    ...
    public static function jalaliToGregorian($jalaliDateString)
    {
        if (!$jalaliDateString || empty(trim($jalaliDateString))) {
            return null;
        }

        // حذف فاصله‌ها و کاراکترهای اضافی
        $jalaliDateString = trim($jalaliDateString);

        // بررسی فرمت YYYY/MM/DD
        if (preg_match('/^(\d{4})\/(\d{1,2})\/(\d{1,2})$/', $jalaliDateString, $matches)) {
            $year = (int)$matches[1];
            $month = (int)$matches[2];
            $day = (int)$matches[3];
