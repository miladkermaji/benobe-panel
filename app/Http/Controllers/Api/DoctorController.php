        ...
        try {
                    ...
                    'doctor'       => function ($query) {
                        $query->select('id', 'first_name', 'last_name', 'specialty_id', 'profile_photo_path', 'province_id')
                            ->with([
                                ...
                                'appointmentConfig', // گرفتن تنظیمات تقویم
                            ]);
                    },
                    'hospital'     => fn($query)     => $query->select('id', 'name'),
                    'appointments' => fn($query) => $query->where('appointments.status', 'scheduled'), // مشخص کردن جدول
                ])
            ...
            $formattedDoctors = $bestDoctors->map(function ($bestDoctor) {
                ...
                return [
                    ...
                    'image'               => $bestDoctor->doctor->profile_photo_path,
...
private function getNextAvailableSlot($doctor)
{
    ...
    $bookedAppointments = Appointment::where('doctor_id', $doctorId)
        ->where('appointments.status', 'scheduled') // مشخص کردن جدول
