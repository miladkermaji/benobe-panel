...
public function store(Request $request)
{
    $user = Auth::user();
    $messages = [
        'doctor_slug.required' => 'انتخاب پزشک الزامی است.',
        'doctor_slug.exists' => 'پزشک انتخاب‌شده معتبر نیست.',
    ...
    $validated = $request->validate([
        'doctor_slug' => 'required|exists:doctors,slug', // تغییر از doctor_id به doctor_slug
    ...
    // یافتن doctor_id بر اساس slug
    $doctor = Doctor::where('slug', $validated['doctor_slug'])->firstOrFail();
    $validated['doctor_id'] = $doctor->id; // افزودن doctor_id به داده‌های معتبر
    unset($validated['doctor_slug']); // حذف doctor_slug از داده‌های معتبر
