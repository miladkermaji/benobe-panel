...
class MakeCrudLivewire extends Command
{
 ...
 protected function appendRoutes($model, $prefix)
 {
  ...
  // پیدا کردن گروه موجود توی فایل routes/web.php
  ...
  if (preg_match($usePattern, $webContent, $matches)) {
   ...
   $webContent = "<?php\n\n$controllerUse" . substr($webContent, 5); // اضافه کردن use در بالای فایل
  }

  // پیدا کردن گروه prefix و اضافه کردن روت‌ها به اون
  $groupPattern = "/Route::prefix\('$prefix'\)\s*->namespace\('$namespacePrefix'\)\s*->group\(function\s*\(\)\s*\{(.*?)\}\);/s";
  if (preg_match($groupPattern, $webContent, $matches)) {
   ...
   if (preg_match($panelPattern, $groupContent, $panelMatches)) {
    // اضافه کردن روت‌ها به گروه panel
    $newPanelContent = $panelMatches[1] . "\n\n" . $routeContent;
    $webContent = str_replace($panelMatches[0], "Route::prefix('panel')->group(function () {\n$newPanelContent\n});", $webContent);
   } else {
    // اگه گروه panel وجود نداشت، به گروه prefix اضافه می‌کنیم
    $newGroupContent = $groupContent . "\n\nRoute::prefix('panel')->group(function () {\n$routeContent\n});";
    $webContent = str_replace($matches[0], "Route::prefix('$prefix')->namespace('$namespacePrefix')->group(function () {{$newGroupContent}\n});", $webContent);
   }
  } else {
   // اگه گروه prefix وجود نداشت، به آخر فایل اضافه می‌کنیم
   $webContent .= "\n\n" . $controllerUse . $routeStub;
  }
