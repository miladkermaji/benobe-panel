...
class LaboratoryEdit extends Component
{
    ...
    public function mount($id)
    {
        ...
        $this->fill($this->laboratory->toArray());
        $this->doctor_ids = $this->laboratory->doctors->pluck('id')->map('strval')->toArray();
...
'service_ids' => $this->service_ids,
    ...
    public function update()
    {
        $validator = Validator::make($this->all(), [
            ...
            'type' => 'required|in:laboratory,treatment_centers,clinic,imaging_center,laboratory,pharmacy',
...
'service_ids.*.exists' => 'خدمت انتخاب‌شده معتبر نیست.',
        ...
        if ($this->avatar) {
            if ($this->laboratory->avatar) {
                Storage::disk('public')->delete($this->laboratory->avatar);
            }
            $data['avatar'] = $this->avatar->store('avatars', 'public');
        }

        if ($this->documents) {
            if ($this->laboratory->documents) {
                foreach ($this->laboratory->documents as $document) {
        ...
        $this->dispatch('show-alert', type: 'success', message: 'آزمایشگاه  با موفقیت به‌روزرسانی شد!');
        return redirect()->route('admin.panel.laboratories.index');
    }

    public function render()
    {
        return view('livewire.admin.panel.laboratories.laboratory-edit');
    }
}
