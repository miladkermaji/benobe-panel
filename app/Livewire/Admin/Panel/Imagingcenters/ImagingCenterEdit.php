...
class ImagingCenterEdit extends Component
{
    ...
    public $is_active, $working_days = [], $gallery = [], $documents = [], $phone_numbers = [];
    ...
    {
        $this->imaging_center = ImagingCenter::findOrFail($id);
        $this->fill($this->imaging_center->toArray());
    ...
    public function update()
    {
        ...
        $this->imaging_center->update($validator->validated());

        $this->dispatch('show-alert', type: 'success', message: 'مرکز تصویربرداری با موفقیت به‌روزرسانی شد!');
