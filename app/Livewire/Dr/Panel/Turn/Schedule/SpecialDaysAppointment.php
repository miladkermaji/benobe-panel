...
public function confirmAddSlot($savePrevious)
{
    ...
    try {
        ...
        // Fetch existing work hours and appointment settings
        $doctorId = $this->getAuthenticatedDoctor()->id;
        ...
        // If not saving previous data, clear existing rows
        if (!$savePrevious) {
            $workHours = [];
            $appointmentSettings = [];
        }
        ...
        $this->dispatch('show-toastr', type: 'error', message: 'خطا در اضافه کردن ردیف جدید: ' . $e->getMessage());
    } finally {
        $this->isProcessing = false;
    }
}
    ...
    private function hasTimeOverlap($newStart, $newEnd, $existingWorkHours)
    {
        ...
        foreach ($existingWorkHours as $slot) {
            if (!empty($slot['start']) && !empty($slot['end'])) {
                ...
                if ($newStart->between($existingStart, $existingEnd) || $newEnd->between($existingStart, $existingEnd) ||
