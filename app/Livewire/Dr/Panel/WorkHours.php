...
public function saveCalculator()
{
    try {
        ...
        try {
            ...
            // بررسی وجود ایندکس برای ویرایش
            if (!isset($workHours[$index])) {
                throw new \Exception('زمان‌بندی با این ایندکس وجود ندارد. لطفاً یک زمان‌بندی موجود را انتخاب کنید.');
            }
            ...
            // فقط اگر زمان‌بندی تغییر کرده باشد، بررسی تطابق و تداخل انجام شود
            if (!$isSameTime) {
            ...
            // ویرایش زمان‌بندی در ایندکس مشخص‌شده
        ...
        $errorMessages = $e->validator->errors()->all();
        $errorMessage = implode('، ', $errorMessages);
        $this->modalMessage = $errorMessage ?: ' Lütfen tüm gerekli alanları doldurun';
        $this->modalType = 'error';
        $this->modalOpen = true;
